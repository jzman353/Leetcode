...
class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        if len(s1)+len(s2) != len(s3):
            return False
        if set(s1).union(set(s2)) != set(s3):
            return False
        self.answer = False
        def helper(s1,s2,s3):
            if s1 == s2 == s3 == "":
                self.answer = True
            if not self.answer and s3:
                if s1 and s3[0] == s1[0]:
                    helper(s1[1:],s2,s3[1:])
            if not self.answer and s3:
                if s2 and s3[0] == s2[0]:
...
Time complexity : O(m \cdot n)O(m⋅n). dp array of size nn is filled mm times.
