...
class BrowserHistory:
    ...
    def visit(self, url: str) -> None:
        self.path = self.path[:self.index+1]+[url]
        self.index += 1
    ...
    def forward(self, steps: int) -> str:
        self.index += min(steps,len(self.path)-1-self.index)
        return self.path[self.index]
...
class BrowserHistory:
    ...
    def back(self, steps: int) -> str:
        i = 0
        gate = min(steps,len(self.backwards)-1)
        while i < gate:
    ...
    def forward(self, steps: int) -> str:
        i = 0
        gate = min(steps,len(self.forwards))
        while i < gate:
