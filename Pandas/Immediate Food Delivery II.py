...
def immediate_food_delivery(delivery: pd.DataFrame) -> pd.DataFrame:
    ...
    first_orders = len(df)
    df = df[df['order_date'] == df['customer_pref_delivery_date']]
    immediate_orders = len(df)
    return pd.DataFrame({"immediate_percentage":[round(100*immediate_orders/first_orders, 2)]})
...
def immediate_food_delivery(delivery: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame({'immediate_percentage': [round(delivery['immediate'].mean() * 100, 2)]})
"""